...
export default function WebdevWritings({ locale }: { locale: Locale }) {
  const dict = getContent(locale);
  const { sectionTitle, filterCategories } = dict.webdev.writings;
  const allowed = new Set<string>(filterCategories);
  const posts = dict.writing.posts.filter((p) => allowed.has(p.category));
